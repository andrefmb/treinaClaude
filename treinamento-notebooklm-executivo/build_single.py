...
FAC_NOTES = {
    "00": build.P("Abra com a pergunta: “Quanto tempo do seu time é gasto lendo relatórios "
                  "antes de decidir — e quanto você confia no resumo de outra pessoa?”. "
                  "Posicione o NotebookLM como o analista que leu tudo e cita a página. "
                  "Apresente as fontes da Aurora e a regra do dado fictício."),
    "02": build.P("<strong>Demo de confiança.</strong> Peça algo que NÃO está nas fontes "
                  "(ex.: faturamento do 1T2026). O NotebookLM deve dizer que não encontrou — "
                  "destaque que esse “não sei” é exatamente o comportamento desejado numa "
                  "ferramenta de decisão."),
    "03": build.P("<strong>Curadoria.</strong> Garanta que todos subam as 4 fontes da Aurora "
                  "num notebook só. Reforce: notebooks focados respondem melhor; menos e "
                  "melhor > muito e disperso."),
    "04": build.P("<strong>O ponto-chave: a citação.</strong> Faça-os clicar nos números das "
                  "respostas e ir ao trecho de origem. A pergunta muda de “será verdade?” "
                  "para “de qual fonte?”. Sempre conferir antes de levar adiante."),
    "06": build.P("<strong>Momento UAU.</strong> Gere um Audio Overview ao vivo e mostre a "
                  "personalização (focar num tema, idioma). Conecte ao dia real deles: ouvir "
                  "o briefing no trajeto. Confirme a disponibilidade de Video Overview no plano."),
    "10": build.P("Deixe claro que <strong>não competem</strong>: NotebookLM para entender com "
                  "confiança; Claude para produzir e automatizar. Mostre o fluxo combinado."),
    "11": build.P("Reforce: o grounding reduz a alucinação, mas <strong>não elimina</strong> — "
                  "uma citação pode embasar mal uma conclusão. Em ambiente corporativo, usar a "
                  "versão aprovada (Workspace). Compartilhar um notebook = compartilhar as fontes."),
    "12": build.P("Cada participante escreve três compromissos (semana / mês / trimestre). "
                  "Entregue a biblioteca de perguntas e o checklist de governança (Recursos)."),
}
...
def section_facil_guide():
    b = ['<h1>Guia do facilitador</h1>',
         build.P("Esta versão é o roteiro de condução. Os blocos "
                 ...
                 "ênfases e momentos de demonstração ao vivo."),
         build.callout("Use fontes públicas ou fictícias. Reforce a regra na abertura e de "
                       "novo na governança: nunca subir dados pessoais (LGPD) ou confidenciais "
                       "em ferramentas não aprovadas.",
                       label="Regra de ouro do dia", kind="warn"),
         build.H2("Antes de começar (setup)"),
         build.P("Garanta que cada participante tenha: acesso ao NotebookLM (login feito) e os "
                 "arquivos do caso Aurora baixados para subir como fontes. Tenha projetor para "
                 "demonstrar o Audio Overview ao vivo."),
         build.H2("Princípios de facilitação"),
         build.table(["Situação", "Como conduzir"],
                     [["O cético", "Faça a demo do “não encontrei nas fontes” e mostre as citações. Confiança vem de ver a origem."],
                      ["O entusiasta", "Desafie a conferir uma citação que embasa mal uma conclusão. O grounding ajuda, não substitui o julgamento."],
                      ["Fluências distintas", "Trabalhe em pares e circule pela sala."],
                      ["O ritual do debrief", "Após cada exercício: “O que economizou tempo?” e “Você conferiu a citação?”."]]),
         build.H2("Sequência sugerida (núcleo)"),
         build.P("Abertura · Conhecendo o NotebookLM · Fundamentos · Montar o notebook · "
                 "Perguntar com citações · Briefing/FAQ/Timeline · Audio Overview · Do notebook "
                 "ao entregável · NotebookLM × Claude · Governança · Plano. Os aprofundamentos "
                 "(Mapa mental, Base de conhecimento do time) ficam bem como sessões à parte. "
                 "Priorize a mão na massa — principalmente o Audio Overview ao vivo."),
         build.callout("As fontes da Aurora, a biblioteca de perguntas, o guia de montagem e o "
                       "checklist de governança estão na seção Recursos.", label="Materiais")]
...
def section_home():
    ...
    body = (
        ...
        "<h1>Dominando o <span style='color:var(--gold);font-style:italic'>NotebookLM</span></h1>"
        '<p class="lede">A ferramenta do Google que lê a SUA pilha de documentos e responde '
        "ancorada nas fontes, com citações verificáveis. Para o executivo: digerir relatórios "
        "em minutos, gerar briefings, ouvir um “podcast” das suas leituras e confiar mais na "
        "resposta. Fio condutor: as fontes da <strong>Aurora S.A.</strong> (as mesmas da "
        "trilha de Claude). Documento único — navegue pela barra lateral; seu progresso fica "
        "salvo neste navegador.</p>"
        '<a class="btn" href="#modulo-00">Começar pelo Módulo 00 →</a> '
        '<a class="btn ghost" href="#dicas">Ver as dicas</a>'
        + build.callout("Use fontes públicas ou fictícias. Nunca suba dados pessoais (LGPD) ou "
        ...
        + build.table(["Ao final, cada participante consegue…", ""],
                      [["Montar um notebook das suas fontes", "relatórios, transcrições, web, áudio."],
                       ...
                       ["Combinar com o Claude e governar", "entender → produzir; com cuidado de LGPD."]])
        + build.H2("A jornada")
        + agenda
...
def section_recursos(facilitator=False):
    b = ['<h1>Recursos para levar</h1>'
         ...
         "materiais do programa — embutidos neste documento.</p>"]
    b.append(build.H2("Fontes do caso Aurora (suba no notebook)"))
    b.append(file_rows(build.DATAROOM_FILES))
    b.append(build.H2("Materiais do programa"))
    b.append(file_rows(build.GEN_FILES))
    if facilitator and build.FAC_FILES:
...
def sidebar(facilitator=False):
    ...
    s = ['<aside class="sidebar"><div class="side-inner">'
         '<a class="brand" href="#inicio"><span class="mark">Dominando o NotebookLM</span></a>'
         + badge
         + '<div class="progress-bar"><span data-progress-fill></span></div>'
         '<p class="progress-text" data-progress-text>0 de 13 módulos</p>'
         '<nav class="toc">'
...
(function(){
  "use strict";
  var KEY="treino-notebooklm-exec-progresso";
  var ORDER=["00","01","02","03","04","05","06","07","08","09","10","11","12"];
...
def build_html(css, facilitator):
    ...
    title = ("Dominando o NotebookLM — guia do facilitador" if facilitator
             else "Dominando o NotebookLM — caderno do participante")
    htmlout = (
        "<!DOCTYPE html>\n"
